...
async function openConversation(id){ state.activeConversation=id; await loadConversations(); const [{data,error},{data:members}]=await Promise.all([supabase.from('messages').select('*').eq('conversation_id',id).order('created_at'),supabase.from('conversation_members').select('user_id').eq('conversation_id',id)]); if(error)return toast(error.message,'err'); const senderIds=[...new Set((data||[]).map(m=>m.sender_id))]; let profiles={}; if(senderIds.length){const {data:p}=await supabase.from('profiles').select('id,full_name').in('id',senderIds);(p||[]).forEach(x=>profiles[x.id]=x.full_name||'Üye');} const other=(members||[]).find(m=>m.user_id!==state.user.id); $('#chatEmpty').classList.add('hidden'); $('#chatActive').classList.remove('hidden'); $('#chatHeader').textContent=other?(profiles[other.user_id]||'İlan görüşmesi'):'İlan görüşmesi'; $('#messageList').innerHTML=(data||[]).map(m=>`<div class="bubble ${m.sender_id===state.user.id?'mine':''}">${m.sender_id!==state.user.id?`<b class="sender-name">${esc(profiles[m.sender_id]||'Üye')}</b>`:''}${esc(m.body)}<small>${new Date(m.created_at).toLocaleString('tr-TR')}</small></div>`).join(''); $('#messageList').scrollTop=$('#messageList').scrollHeight; }
