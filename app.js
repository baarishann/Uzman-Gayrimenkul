...
async function openListing(id){
  ...
  if(!l) return;
  addRecent(id);
  let imgs=l.images||[]; if(!imgs.length){const {data:im}=await supabase.from('listing_images').select('*').eq('listing_id',l.id).order('sort_order');imgs=im||[];l.images=imgs;} const hero=imgs.find(x=>x.is_cover)?.image_url||imgs[0]?.image_url; const gallery=imgs.length?`<div class="detail-gallery" data-gallery>${imgs.map((im,i)=>`<div class="gallery-slide"><img src="${esc(im.image_url)}" alt="${esc(l.title)} ${i+1}"><span>${i+1}/${imgs.length}</span></div>`).join('')}</div>`:(hero?`<div class="detail-hero"><img src="${esc(hero)}" alt="${esc(l.title)}"></div>`:`<div class="detail-hero">${placeholder()}</div>`);
  ...
  $('#listingDetail').innerHTML=`${gallery}<div class="detail-content">
    ...
    <div class="detail-grid"><div><div class="detail-box"><div class="detail-facts">${details.map(([k,v])=>`<div class="detail-fact"><small>${esc(k)}</small><b>${esc(v)}</b></div>`).join('')}</div></div>
      ...
      <aside><div class="detail-box contact-card"><div class="eyebrow">İLAN İLETİŞİMİ</div><h3>${esc(l.contact_name||'Uzman Emlak')}</h3><p class="location">${esc(l.contact_phone||'0506 134 76 75')}</p>
        ...
        <button class="ghost-btn" data-share-listing="${l.id}">İlanı Paylaş</button><button class="ghost-btn" data-report-listing="${l.id}">İlanı Şikayet Et</button>
      </div></aside></div></div>`;
...
function fillProfile(){ $('#profileName').value=state.profile?.full_name||''; $('#profilePhone').value=state.profile?.phone||''; $('#profileCompany').value=state.profile?.company_name||''; $('#profileBio').value=state.profile?.bio||''; const a=$('#profileAvatar'); if(a){a.innerHTML=state.profile?.avatar_url?`<img src="${esc(state.profile.avatar_url)}" alt="Profil">`:esc((state.profile?.full_name||'Ü').trim().charAt(0).toLocaleUpperCase('tr'));} }
...
async function uploadListingImages(listingId,files){
  const {data:existing}=await supabase.from('listing_images').select('id,sort_order').eq('listing_id',listingId).order('sort_order',{ascending:false}).limit(1); let order=existing?.length?Number(existing[0].sort_order||0)+1:0; for(const file of files){ if(file.size>10*1024*1024){toast(`${file.name} 10 MB sınırını aşıyor.`,'err');continue;} const ext=(file.name.split('.').pop()||'jpg').toLowerCase(); const path=`${state.user.id}/${listingId}/${crypto.randomUUID()}.${ext}`; const {data,error}=await supabase.storage.from('listing-images').upload(path,file,{cacheControl:'3600',upsert:false,contentType:file.type}); if(error){toast(`Fotoğraf yüklenemedi: ${error.message}`,'err');continue;} const {data:urlData}=supabase.storage.from('listing-images').getPublicUrl(data.path); const {error:dbErr}=await supabase.from('listing_images').insert({listing_id:listingId,image_url:urlData.publicUrl,sort_order:order,is_cover:order===0}); if(dbErr)toast(dbErr.message,'err'); order++; }
}
...
async function uploadProfilePhoto(file){ if(!state.user||!file)return; if(file.size>5*1024*1024)return toast('Profil fotoğrafı en fazla 5 MB olabilir.','err'); const ext=(file.name.split('.').pop()||'jpg').toLowerCase(); const path=`avatars/${state.user.id}/${crypto.randomUUID()}.${ext}`; const {data,error}=await supabase.storage.from('listing-images').upload(path,file,{cacheControl:'3600',upsert:false,contentType:file.type}); if(error)return toast(error.message,'err'); const {data:url}=supabase.storage.from('listing-images').getPublicUrl(data.path); const {data:p,error:e}=await supabase.from('profiles').update({avatar_url:url.publicUrl}).eq('id',state.user.id).select().single(); if(e)return toast(e.message,'err');state.profile=p;fillProfile();updateAuthUI();toast('Profil fotoğrafı güncellendi.');}
async function sendFeedback(e){e.preventDefault();if(!state.user)return openAuth();const reason=$('#feedbackType').value,details=$('#feedbackDetails').value.trim();if(!details)return;const {error}=await supabase.from('reports').insert({reporter_id:state.user.id,reason,details,status:'open'});if(error)return toast(error.message,'err');e.target.reset();toast('Bildiriminiz gönderildi.');}
async function reportListing(id){if(!state.user)return openAuth();const details=prompt('İlanla ilgili şikayetinizi kısaca yazın:');if(!details)return;const {error}=await supabase.from('reports').insert({reporter_id:state.user.id,listing_id:id,reason:'listing_complaint',details,status:'open'});if(error)return toast(error.message,'err');toast('İlan şikayeti gönderildi.');}
async function deleteMyAccount(){if(!state.user)return;if(!confirm('Hesabınız kalıcı olarak silinecek. Bu işlem geri alınamaz. Devam edilsin mi?'))return;const {error}=await supabase.functions.invoke('delete-my-account',{body:{confirm:true}});if(error)return toast(error.message,'err');await supabase.auth.signOut();nav('home');toast('Hesabınız silindi.');}
...
function nav(name){
  ...
  $$('.view').forEach(v=>v.classList.remove('active-view')); const id={home:'homeView',clients:'clientsView',tasks:'tasksView',evaluation:'evaluationView',favorites:'favoritesView',messages:'messagesView',account:'accountView'}[name]||'homeView'; $('#'+id).classList.add('active-view');
  $$('.bottom-nav button').forEach(b=>b.classList.toggle('active',b.dataset.nav===name));
  if(name==='clients'||name==='tasks')CRM.render(); if(name==='evaluation')renderDailyReview(); if(name==='favorites')renderFavoriteView(); if(name==='messages')loadConversations(); if(name==='account')hydrateAccount(); window.scrollTo({top:0,behavior:'smooth'});
}
...
const CRM={
 ...
 render(){
  ...
  let matches=0;c.filter(x=>x.type==='Alıcı'||x.type==='Kiracı').forEach(x=>{matches+=state.listings.filter(l=>l.status==='active'&&(!x.property||l.category===x.property)&&(!x.max||!l.price||Number(l.price)<=Number(x.max))).length});set('crmMatchCount',matches);set('focusTaskCount',t.filter(x=>!x.done).length);
...
window.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>CRM.render(),300);document.getElementById('crmClientForm')?.addEventListener('submit',e=>{e.preventDefault();CRM.clients.push({id:CRM.id(),name:ccName.value,phone:ccPhone.value,type:ccType.value,status:ccStatus.value,min:ccMin.value,max:ccMax.value,property:ccProperty.value,area:ccArea.value,rooms:(document.getElementById('ccRooms')?.value||''),follow:(document.getElementById('ccFollow')?.value||''),notes:ccNotes.value,created:new Date().toISOString()});CRM.save();CRM.render();e.target.reset();crmClientDialog.close();toast('Müşteri kaydedildi.');});document.getElementById('crmTaskForm')?.addEventListener('submit',e=>{e.preventDefault();const c=CRM.clients.find(x=>x.id===ctClient.value);CRM.tasks.push({id:CRM.id(),title:ctTitle.value,clientId:ctClient.value,clientName:c?.name||'',date:ctDate.value,priority:ctPriority.value,notes:ctNotes.value,done:false});CRM.save();CRM.render();e.target.reset();crmTaskDialog.close();toast('Takip oluşturuldu.');});document.addEventListener('click',e=>{const o=e.target.closest('[data-crm-open]');if(o){CRM.open(o.dataset.crmOpen);return}const d=e.target.closest('[data-crm-del-client]');if(d&&confirm('Müşteri silinsin mi?')){CRM.clients=CRM.clients.filter(x=>x.id!==d.dataset.crmDelClient);CRM.save();CRM.render()}const td=e.target.closest('[data-crm-task-done]');if(td){const x=CRM.tasks.find(x=>x.id===td.dataset.crmTaskDone);if(x)x.done=!x.done;CRM.save();CRM.render()}})});

/* === Uzman Emlak CRM v52 enhancements === */
const CRM52={
 score(c){let s=25;if(c.status==='Sıcak')s+=35;if(c.status==='Teklif Verdi')s+=50;if(c.status==='Takipte')s+=15;if(c.phone)s+=8;if(c.max)s+=8;if(c.area)s+=6;if(c.notes)s+=4;return Math.min(100,s)},
 scoreLabel(n){return n>=75?'Yüksek':n>=50?'Orta':'Yeni'},
 wa(phone){const p=String(phone||'').replace(/\D/g,'');if(!p)return '#';const x=p.startsWith('90')?p:p.startsWith('0')?'9'+p:'90'+p;return `https://wa.me/${x}`},
 filtered(){const q=(document.getElementById('crmClientSearch')?.value||'').toLocaleLowerCase('tr-TR');const f=document.getElementById('crmClientFilter')?.value||'';return CRM.clients.filter(c=>(!q||`${c.name} ${c.phone||''} ${c.area||''}`.toLocaleLowerCase('tr-TR').includes(q))&&(!f||c.status===f))},
 renderClients(){const cl=document.getElementById('crmClientsList');if(!cl)return;const data=this.filtered().slice().reverse();cl.innerHTML=data.map(c=>{const sc=this.score(c);return `<div class="crm-row"><div><b>${esc(c.name)}</b> <span class="crm-score">${sc}/100 • ${this.scoreLabel(sc)}</span><p>${esc(c.phone||'Telefon yok')} • ${esc(c.type)} • ${esc(c.status)} • ${esc(c.area||'Bölge yok')}</p><small>${c.follow?`Takip: ${new Date(c.follow).toLocaleString('tr-TR')} • `:''}${esc(c.notes||'')}</small></div><div class="crm-row-actions">${c.phone?`<a class="mini-btn" href="tel:${esc(c.phone)}">Ara</a><a class="mini-btn ok" target="_blank" rel="noopener" href="${this.wa(c.phone)}">WhatsApp</a>`:''}<button class="mini-btn" data-crm-quick-task="${c.id}">Takip</button><button class="mini-btn warn" data-crm-del-client="${c.id}">Sil</button></div></div>`}).join('')||'<div class="crm-empty">Müşteri bulunamadı.</div>';this.summary()},
 summary(){const now=new Date(),end=new Date();end.setHours(23,59,59,999);let due=0,over=0;CRM.clients.forEach(c=>{if(!c.follow)return;const d=new Date(c.follow);if(d>=new Date(now.getFullYear(),now.getMonth(),now.getDate())&&d<=end)due++;if(d<now)over++});const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('crmDueToday',due);set('crmOverdue',over);set('crmHighScore',CRM.clients.filter(c=>this.score(c)>=75).length)},
 pipeline(){const stages=['Yeni','Takipte','Sıcak','Teklif Verdi','Sonuçlandı'];const box=document.getElementById('crmPipeline');if(box)box.innerHTML=stages.map(s=>`<div class="crm-stage"><b>${CRM.clients.filter(c=>c.status===s).length}</b><small>${s}</small></div>`).join('');document.getElementById('crmPipelineDialog')?.showModal()},
 importFile(file){if(!file)return;const info=document.getElementById('crmImportInfo');if(info)info.textContent='Dosya okunuyor...';const r=new FileReader();r.onload=()=>{const text=String(r.result||'');const lines=text.split(/\r?\n/).filter(Boolean);const first=(lines[0]||'').split(/[;,]/).map(x=>x.trim());if(info)info.innerHTML=`<b>${lines.length>0?lines.length-1:0}</b> veri satırı bulundu.<br>Algılanan sütunlar: ${first.slice(0,12).map(esc).join(', ')}<br><small>Aktarım eşleştirmesi dosya formatına göre bir sonraki adımda onaylanır.</small>`};r.readAsText(file,'utf-8')}
};
window.addEventListener('DOMContentLoaded',()=>{
 setTimeout(()=>{CRM52.renderClients();CRM52.summary()},450);
 document.getElementById('crmClientSearch')?.addEventListener('input',()=>CRM52.renderClients());
 document.getElementById('crmClientFilter')?.addEventListener('change',()=>CRM52.renderClients());
 document.getElementById('crmCommissionForm')?.addEventListener('submit',e=>{e.preventDefault();const p=Number(document.getElementById('cmPrice').value||0),r=Number(document.getElementById('cmRate').value||0),v=Number(document.getElementById('cmVat').value||0);const base=p*r/100,total=base*(1+v/100);document.getElementById('cmResult').textContent=new Intl.NumberFormat('tr-TR',{style:'currency',currency:'TRY',maximumFractionDigits:0}).format(total)});
 document.getElementById('crmImportFile')?.addEventListener('change',e=>CRM52.importFile(e.target.files?.[0]));
 document.addEventListener('click',e=>{const q=e.target.closest('[data-crm-quick-task]');if(q){const c=CRM.clients.find(x=>x.id===q.dataset.crmQuickTask);if(c){document.getElementById('ctClient').value=c.id;document.getElementById('ctTitle').value=`${c.name} ile görüş`;document.getElementById('crmTaskDialog').showModal()}}});
});
const _crmRender=CRM.render.bind(CRM);CRM.render=function(){_crmRender();setTimeout(()=>CRM52.renderClients(),0)};
const _crmOpen=CRM.open.bind(CRM);CRM.open=function(type){if(type==='pipeline')return CRM52.pipeline();if(type==='commission')return document.getElementById('crmCommissionDialog')?.showModal();if(type==='import')return document.getElementById('crmImportDialog')?.showModal();return _crmOpen(type)};

// v53 daily performance
const DAILY_KEY='uzman-emlak-daily-review';
function getDaily(){try{return JSON.parse(localStorage.getItem(DAILY_KEY)||'{}')}catch{return {}}}
function renderDailyReview(){const d=getDaily(),key=new Date().toISOString().slice(0,10),x=d[key]||{calls:0,meetings:0,visits:0,stars:0,notes:''}; const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('dailyCalls',x.calls||0);set('dailyMeetings',x.meetings||0);set('dailyVisits',x.visits||0);const dt=document.getElementById('dailyDate');if(dt)dt.textContent=new Date().toLocaleDateString('tr-TR',{weekday:'long',day:'numeric',month:'long',year:'numeric'});document.querySelectorAll('#dailyStars [data-star]').forEach(b=>b.classList.toggle('on',Number(b.dataset.star)<=Number(x.stars||0)));const n=document.getElementById('dailyNotes');if(n)n.value=x.notes||''}
function updateDaily(field,step){const d=getDaily(),key=new Date().toISOString().slice(0,10),x=d[key]||{calls:0,meetings:0,visits:0,stars:0,notes:''};x[field]=Math.max(0,Number(x[field]||0)+step);d[key]=x;localStorage.setItem(DAILY_KEY,JSON.stringify(d));renderDailyReview()}
document.addEventListener('click',e=>{const b=e.target.closest('[data-daily]');if(b)updateDaily(b.dataset.daily,Number(b.dataset.step));const st=e.target.closest('[data-star]');if(st){const d=getDaily(),key=new Date().toISOString().slice(0,10),x=d[key]||{calls:0,meetings:0,visits:0,stars:0,notes:''};x.stars=Number(st.dataset.star);d[key]=x;localStorage.setItem(DAILY_KEY,JSON.stringify(d));renderDailyReview()}});
document.getElementById('saveDailyBtn')?.addEventListener('click',()=>{const d=getDaily(),key=new Date().toISOString().slice(0,10),x=d[key]||{calls:0,meetings:0,visits:0,stars:0,notes:''};x.notes=document.getElementById('dailyNotes')?.value||'';d[key]=x;localStorage.setItem(DAILY_KEY,JSON.stringify(d));toast('Gün değerlendirmesi kaydedildi.')});
