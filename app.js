...
function filteredListings(){
  ...
  if(f.q){ const q=f.q.toLocaleLowerCase('tr'); arr=arr.filter(l=>[l.title,l.description,l.neighborhood,l.ada,l.parsel,l.subcategory].some(v=>String(v||'').toLocaleLowerCase('tr').includes(q))); }
...
async function loadConversations(){
  ...
  $('#conversationList').innerHTML=arr.map(c=>{const l=listings[c.listing_id]; return `<button class="conversation-item ${state.activeConversation===c.id?'active':''}" data-conversation="${c.id}"><b>${esc(l?.title||'İlan görüşmesi')}</b><small>${l?`İlan No ${l.listing_no}`:'Özel görüşme'} • ${new Date(c.updated_at).toLocaleDateString('tr-TR')}</small></button>`}).join('')||'<div class="empty-state compact"><div>✉</div><h3>Mesaj yok</h3></div>';
}
async function openConversation(id){ state.activeConversation=id; await loadConversations(); const {data,error}=await supabase.from('messages').select('*').eq('conversation_id',id).order('created_at'); if(error)return toast(error.message,'err'); $('#chatEmpty').classList.add('hidden'); $('#chatActive').classList.remove('hidden'); $('#chatHeader').textContent='İlan görüşmesi'; $('#messageList').innerHTML=(data||[]).map(m=>`<div class="bubble ${m.sender_id===state.user.id?'mine':''}">${esc(m.body)}<small>${new Date(m.created_at).toLocaleString('tr-TR')}</small></div>`).join(''); $('#messageList').scrollTop=$('#messageList').scrollHeight; }
